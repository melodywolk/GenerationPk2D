...
def gfield2d(N,k,Pk,V):
 ...
 re[N2+1:N,0] =  np.flipud(re[1:N2,0])
 im[N2+1:N,0] = -np.flipud(im[1:N2,0])
 re[0,N2+1:N] =  np.flipud(re[0,1:N2])
 im[0,N2+1:N] = -np.flipud(im[0,1:N2])

 
 for i in range(1,N2):
     re[N-i,N2+1:N] =  np.flipud(re[i,1:N2])
     im[N-i,N2+1:N] = -np.flipud(im[i,1:N2])
     re[i,N2+1:N]   =  np.flipud(re[N-i,1:N2])
     im[i,N2+1:N]   = -np.flipud(im[N-i,1:N2])
  
 re[N2,N2+1:N] =  np.flipud(re[N2,1:N2]) 
 im[N2,N2+1:N] = -np.flipud(im[N2,1:N2])
 re[N2+1:N,N2] =  np.flipud(re[1:N2,N2])
 im[N2+1:N,N2] = -np.flipud(im[1:N2,N2])
